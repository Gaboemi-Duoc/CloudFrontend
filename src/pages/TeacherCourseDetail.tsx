...
export default function TeacherCourseDetail() {
  ...
  useEffect(() => { if (token && courseId) void reload(); }, [token, courseId]);
  ...
  return (
    <section>
      ...
      <div className="card">
        ...
        <ul>
          {students.map((s) => <li key={s.id}>{s.studentId}</li>)}
      ...
      <div className="card">
        <h3>Create a task</h3>
        <form onSubmit={(e) => { e.preventDefault(); void onCreateTask(); }}>
      ...
      <div className="card">
        ...
        <ul>
          {tasks.map((t) => (
            <li key={t.id}>
              <b>{t.title}</b>{t.dueDate ? ` — due ${t.dueDate}` : ""}
